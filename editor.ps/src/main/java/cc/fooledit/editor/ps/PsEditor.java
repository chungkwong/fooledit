...
public class PsEditor implements DataEditor<PsObject>{
	...
	public String getName(){
		return MessageRegistry.getString("PS_EDITOR",Activator.class);
	}
	private void addCommand(String name,Consumer<PsViewer> action){
		commandRegistry.put(name,new Command(name,()->action.accept((PsViewer)Main.INSTANCE.getCurrentNode()),Activator.class));
	}
	private void addCommand(String name,List<Argument> parameters,BiFunction<Object[],PsViewer,Object> action){
		commandRegistry.put(name,new Command(name,parameters,(args)->action.apply(args,(PsViewer)Main.INSTANCE.getCurrentDataEditor()),Activator.class));
	}
	@Override
