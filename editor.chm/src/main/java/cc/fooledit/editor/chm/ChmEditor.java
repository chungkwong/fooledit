...
public class ChmEditor implements DataEditor<ChmObject>{
	...
	public String getName(){
		return MessageRegistry.getString("CHM_EDITOR",Activator.class);
	}
	private void addCommand(String name,Consumer<ChmViewer> action){
		commandRegistry.put(name,new Command(name,()->action.accept((ChmViewer)Main.INSTANCE.getCurrentNode()),Activator.class));
	}
	private void addCommand(String name,List<Argument> parameters,BiFunction<Object[],ChmViewer,Object> action){
		commandRegistry.put(name,new Command(name,parameters,(args)->action.apply(args,(ChmViewer)Main.INSTANCE.getCurrentDataEditor()),Activator.class));
	}
	@Override
